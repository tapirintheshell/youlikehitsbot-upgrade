        ...
        function initAutoClose() {
            ...
            GM.getValue("currentVideoInfo", null)
                ...
                if (data) {
                    ...
                    console.log("Alınan video bilgileri:", videoInfo);
                    ...
                    console.log("Video bilgisi bulunamadı. Varsayılan süre kullanılıyor.");
                    timeout_ms = 15 * 60 * 1000; // Varsayılan 15 dakika
                }
                console.log("Auto-close süresi (ms): " + timeout_ms);
                ...
                let intervalId = setInterval(function() {
                    ...
                    if (remaining <= 0) {
                        ...
                        let s = totalSec % 60;
                        countdownDiv.innerHTML = "Pencere " + m + " dakika " + (s < 10 ? "0" : "") + s + " saniye sonra kapanacak.";
                    }
                }, 1000);

                console.log("Auto-close timer başlatıldı.");
            })
                .catch(err => {
            ...
            if (onclickAttr) {
                ...
                if (match && match.length >= 4) {
                    ...
                    GM.setValue("currentVideoInfo", JSON.stringify(videoInfo))
                        .then(() => {
                        console.log("Video bilgileri saklandı.");
                    })
                        .catch(err => {
                        console.error("Veri saklanırken hata:", err);
                    });

                    console.log("Video bilgileri yakalandı ve GM_setValue'a yazıldı:", videoInfo);
                } else {
                    console.log("Onclick attribute ayrıştırılamadı:", onclickAttr);
                }
            }
        }
    }
