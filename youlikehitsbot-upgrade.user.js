...
// @version      0.5.1t
// @description  This project is a modified version of the original YouLikeHits Bot by gekkedev. It has been tailored exclusively for handling YouTube videos in the YouLikeHits workflow.
// @author       tapirintheshell
...
(() => {
    ...
    if (window.location.hostname.indexOf("youtube.com") !== -1) {
        function initAutoClose() {
            ...
            GM.getValue("currentVideoInfo", null)
                ...
                // updater function: calculate time
                let intervalId = setInterval(function() {
                    let remaining = endTime - Date.now();
                    //console.log("Update: remaining ms = " + remaining);
                    if (remaining <= 0) {
                        clearInterval(intervalId);
                        countdownDiv.innerHTML = "Closing...";
                        console.log("Times up, window closing.");
                        window.close();
                    } else {
                        let totalSec = Math.floor(remaining / 1000);
                        let m = Math.floor(totalSec / 60);
                        let s = totalSec % 60;
                        countdownDiv.innerHTML = "Window close in " + m + " minute " + (s < 10 ? "0" : "") + s + " seconds.";
                    }
                }, 1000);
    ...
    // catch followbutton link onclick attribute > video data.
    function captureVideoInfo() {
        // if followbutton exist
        if (J(".followbutton").length) {
            let followBtn = J(".followbutton").first();
            let onclickAttr = followBtn.attr("onclick");
            if (onclickAttr) {
                // sample onclick: imageWin(3919260,'XtWPIKr9ATo','45','7d581c02541d3765b63b8d204e4abdd95f9c1cdf3614cf1c3b9485b68a9bd814');
                let match = onclickAttr.match(/imageWin\((\d+),\s*'([^']+)',\s*'(\d+)'/);
                if (match && match.length >= 4) {
                    let videoId = match[1]; // "3919260"
                    let duration = parseInt(match[3], 10); // 45 sec
                    let videoInfo = {
                        videoId: videoId,
                        duration: duration, // total video time
                        capturedAt: Date.now() //  current timestamp
                    };

                    // store video data (asenkron API)
                ...
                switch (document.location.pathname) {
					...
					case "/youtubenew2.php":
						...
						if (J(".followbutton").length) { //if false, there is likely a captcha waiting to be solved
							...
							let patienceKiller = (prev) => { setTimeout( () => { if (vidID() == prev) { J(".followbutton").parent().children("a:contains('Skip')").click(); newWin.close(); }}, 1000 * 600)}; //max time: 120s + 15s grace time (max length: http://prntscr.com/q4o75o)
							...
							if (vidID() != previousVideo) { //has a new video has been provided yet? This will overcome slow network connections causing the same video to be played over and over
								previousVideo = vidID();
								 ...
								 captureVideoInfo(); // get video data
